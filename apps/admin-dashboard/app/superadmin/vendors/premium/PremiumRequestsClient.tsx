...
export default function PremiumRequestsClient({ initialRequests }: { initialRequests: any[] }) {
  const handleStatusUpdate = async (id: string, status: 'APPROVED' | 'REJECTED') => {
    if (confirm(`Voulez-vous vraiment ${status === 'APPROVED' ? 'approuver' : 'rejeter'} cette demande ?`)) {
      await updateVendorPremiumStatusAction(id, status);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-10">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900/50 backdrop-blur-md p-8 rounded-[32px] border border-white dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none">
        <div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Demandes Premium Vendeur</h1>
          <p className="mt-2 text-slate-500 font-medium tracking-tight">Gérez les demandes de statut Premium pour les fournisseurs marketplace.</p>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-8">
              <div className="flex gap-6">
                <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-500/10 rounded-3xl flex items-center justify-center text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform">
                  <Star size={32} fill={req.status === 'APPROVED' ? 'currentColor' : 'none'} />
                </div>
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-2xl font-black text-slate-900 dark:text-white">{req.vendor.companyName}</h3>
                  ...
                  <div className="text-slate-500 font-medium flex items-center gap-4 text-sm">
                    <span className="flex items-center gap-1"><Phone size={14} /> {req.phone}</span>
                    <span className="flex items-center gap-1"><MessageSquare size={14} /> {req.preferredContact}</span>
                  </div>
                  <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl text-sm text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-slate-800 italic">
              ...
              <div className="flex flex-col justify-center gap-3">
                {req.status === 'PENDING' && (
                  <>
                    <button 
                      ...
                      className="px-8 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-500 shadow-lg shadow-indigo-600/20 transition-all flex items-center gap-2 justify-center"
                    >
                ...
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest text-center">
                  Reçue le {new Date(req.createdAt).toLocaleDateString('fr-FR')}
                </div>
              </div>
            </div>
          </div>
