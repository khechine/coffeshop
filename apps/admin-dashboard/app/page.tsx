...
export default function HomePage() {
  ...
  useEffect(() => {
    if (showContactForm) {
      setCaptchaParams({ n1: Math.floor(Math.random() * 10) + 1, n2: Math.floor(Math.random() * 10) + 1 });
    }
  }, [showContactForm]);
              ...
              {contactSuccess ? (
                ...
                <div style={{ animation: 'fadeInUp 0.3s ease-out' }}>
                  ...
                  <form onSubmit={async (e) => {
                    ...
                    // Captcha Math Validation
                    const userCaptcha = parseInt(formData.get('captcha') as string, 10);
                    if (userCaptcha !== captchaParams.n1 + captchaParams.n2) {
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: 900, color: '#374151', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Adresse E-mail</label>
                      <input name="email" required type="email" placeholder="contact@entreprise.com" style={{ width: '100%', padding: '16px 20px', borderRadius: '16px', border: '2px solid #F1F5F9', outline: 'none', fontSize: '15px', background: '#F8FAFC', transition: 'all 0.2s', fontWeight: 500 }} />
                    </div>
                    <div>
                    ...
                    {/* Captcha Field */}
                    <div style={{ background: '#FEF2F2', padding: '16px', borderRadius: '16px', border: '1px dashed #FCA5A5' }}>
                      <label style={{ display: 'block', fontSize: '13px', fontWeight: 900, color: '#991B1B', marginBottom: '8px' }}>Anti-Spam : Combien font {captchaParams.n1} + {captchaParams.n2} ?</label>
                      <input name="captcha" required type="number" placeholder="Votre réponse" style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid #FECACA', outline: 'none', fontSize: '15px', fontWeight: 800 }} />
                    </div>
