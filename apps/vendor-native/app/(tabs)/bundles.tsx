  ...
  const fetchData = async (vid: string) => {
    try {
      const [bundlesData, productsData] = await Promise.all([
        ApiService.get(`/management/vendor/bundles/${vid}`),
        ApiService.get(`/management/marketplace/products?vendorId=${vid}`)
      ]);
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalForm}>
              ...
              <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Ex: Pack Découverte" placeholderTextColor="#64748b" />
              
              <Text style={styles.label}>Description</Text>
              <TextInput style={[styles.input, { height: 80 }]} value={description} onChangeText={setDescription} multiline placeholder="Détails de l'offre..." placeholderTextColor="#64748b" />
              ...
              <View style={styles.imageInputRow}>
                <TextInput style={[styles.input, { flex: 1, marginBottom: 0 }]} value={newImageUrl} onChangeText={setNewImageUrl} placeholder="Lien vers image..." placeholderTextColor="#64748b" />
...
const styles = StyleSheet.create({
  ...
  modalContent: { backgroundColor: '#0f172a', borderTopLeftRadius: 30, borderTopRightRadius: 30, height: '90%', padding: 25 },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 25, backgroundColor: 'transparent' },
  modalTitle: { fontSize: 20, fontWeight: '900', color: '#fff' },
  modalForm: { flex: 1 },
  label: { fontSize: 13, fontWeight: '700', color: '#94a3b8', marginBottom: 8, marginTop: 15 },
  input: { backgroundColor: 'rgba(255,255,255,0.03)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 15, color: '#fff', marginBottom: 10 },
  ...
  productPicker: { flexDirection: 'row', backgroundColor: 'transparent' },
  pickerItem: { backgroundColor: 'rgba(255,255,255,0.05)', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, marginRight: 10, flexDirection: 'row', alignItems: 'center', gap: 5 },
  pickerItemText: { color: '#94a3b8', fontSize: 13 },
  saveBtn: { backgroundColor: '#f59e0b', padding: 20, borderRadius: 20, alignItems: 'center', marginTop: 10 },
